...
int main(int argc, char *argv[]){                                  
    ...
    send(client_fd, &req_file_name_len, sizeof(int), MSG_NOSIGNAL);
    //再发送文件名的实际字符串内容
    send(client_fd, req_file_name, req_file_name_len, MSG_NOSIGNAL);
    ...
    //接收文件内容并写入本地磁盘 (使用 mmap 内存映射提高写入效率)
    //在本地以读写模式创建（或覆盖）同名文件，权限为 0600 (属主可读可写) 
    int file_fd = open(file_name, O_RDWR|O_CREAT, 0600);
    
    //刚刚创建的文件的大小是0,必须将其截断(扩大)到服务器告知的文件大小
    //否则后面的mmap会因为越界访问而导致段错误(Segmentation fault)
