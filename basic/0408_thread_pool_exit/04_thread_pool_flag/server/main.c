...
int main(int argc, char *argv[]){                                  
    ...
    if(fork() != 0){
        //父进程
        signal(2, func);
        wait(NULL);//父进程阻塞在这里，等着子进程死掉
        exit(0);   //等到子进程死了，父进程就退出
    }

    //让子进程脱离进程组,并成为新的进程组的组长
    //新的进程组的id就是该子进程的id
    //因为父进程是前台进程组,而一个会话中最多只有一个前台进程组,
    //所以此时子进程对应的进程组会自动变成后台进程组,
    //后台进程组在一个会话中可以有很多个
    setpgid(0, 0);

    //线程池中的线程创建出来
    //然后互斥锁,条件变量等都要初始化(也就是初始化线程)
    thread_pool_t pool;
